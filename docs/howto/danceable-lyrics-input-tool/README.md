...
```python
    def __init__(self, provider: AMPProviderV2) -> None:
        ...
        self.DATASETS_BASE = Path("c:/users/alteryx/DanceableLyricsData/").  # please change this the un-zipped location.
```
...
```python
    ...
    def _validate_datasets_dir(datasets_dir: Path) -> None:
        ...
        filenames = [
            ...
            "tracks.csv",
            "r_track_artist.csv",
            "genius_song_lyrics_TRUNCATED.csv",  # for non truncated use  "genius_song_lyrics.csv"
            "audio_features.csv",
        ]
